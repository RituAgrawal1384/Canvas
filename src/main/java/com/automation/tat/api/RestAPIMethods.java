package com.automation.tat.api;
...
//            response.then().log().ifError();
    ...
    public  void restProxySetter() {
        if (Constants.PROXY_URL != null && (configvariable.isProxyRequired())) {
            RestAssured.proxy(ProxySpecification.host(Constants.PROXY_URL));
            RestAssured.proxy(ProxySpecification.port(Integer.parseInt(Constants.PROXY_PORT)));
        }
    }
